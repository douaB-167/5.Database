...
-- sintassi in order sbagliata, corretta
-- order by `cognome`
-- manca la condizione, il filtro
-- where cognome = 'bianchi'
/* select *
from studenti
where cognome = 'bianchi'
order by cognome; */
-- però c'è un mio errore, chiedo:
-- il cui cognome è uguale a 'bianchi' e ordina per cognome
-- quindi non ha senso ordinare per cognome
...
-- order by provincia, citta, cognome, nome;
...
-- consegna: la cui provincia è Cuneo o Alessandria...
-- where provincia = 'cn' or provincia = 'al'
-- il campo provincia è definito come char(2)
-- consegna: ordina per provincia, cognome e nome =>
...
-- genere è definito come enum('f','m','nb') =>
-- and `genere` = 'm'
...
-- regexp si usa per pattern che non si riescono a selezionare con LIKE
